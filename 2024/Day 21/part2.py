...
def compute_sequences(keypad):
    ...
    for r in range(len(keypad)):
        for c in range(len(keypad[r])):
            if keypad[r][c] != None: pos[keypad[r][c]] = (r, c)
    seqs = {}
    for x in pos:
        for y in pos:
            if x == y:
...
for line in open("2024/input.txt").read().splitlines():
    inputs = solve(line, num_seqs)
    length = min([compute_length(seq) for seq in inputs])
